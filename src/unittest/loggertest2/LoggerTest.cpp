...
static volatile char main_sccs_id[] __attribute__ ((unused)) = "@(#)Logger Test 2"
...
void LoggerTest::performTests()
{
   ...
   for (i = 0; i < 100; i++)
   {
      TRACELOG(DEBUGLOG, OPMLOG, "Performing Shared Memory Logger Test for TRACELOG, iteration %d", i,0,0,0,0,0);
      sleep(2);
   }//end for

   ostringstream ostr;
   string s = "iteration ";
   ostr << "Performing Shared Memory Logger Test for STRACELOG, " << s << i++ << ends;
   STRACELOG(DEBUGLOG, OPMLOG, ostr.str().c_str());

   sleep(2);

   ostringstream ostr2;
   ostr << "Performing Another Shared Memory Logger Test for STRACELOG, " << s << i++ << ends;
   STRACELOG(DEBUGLOG, OPMLOG, ostr.str().c_str());
...
int main(int argc, char* argv[])
{
   ...
   // memory queue for the Log Processor to dequeue them
   ...
   cout << "LoggerTest2: Preparing to enqueue logs into shared memory. Please start the Logger for dequeue" 
        << endl;
