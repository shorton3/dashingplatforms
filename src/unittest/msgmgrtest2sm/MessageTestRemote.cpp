...
#include "platform/msgmgr/LocalSMMailbox.h"
...
static volatile char main_sccs_id[] __attribute__ ((unused)) = "@(#)MsgMgr Test 2 SM"
...
MessageTestRemote::MessageTestRemote()
   :messageTestSMMailbox_(NULL),
    messageHandlerList_(NULL)
{
   // Create the LocalMailbox Address
   MailboxAddress localAddress;
   localAddress.locationType = LOCAL_SHARED_MEMORY_MAILBOX;
   localAddress.mailboxName = "MessageTestSM";

   // Setup the local Mailbox
   if (!setupMailbox(localAddress))
   {
      TRACELOG(ERRORLOG, MSGMGRLOG, "Unable to setup local shared memory mailbox",0,0,0,0,0,0);
   }//end if
   else
   {
      TRACELOG(DEBUGLOG, MSGMGRLOG, "MessageTestRemote local shared memory mailbox setup successful",0,0,0,0,0,0);
   }//end else
...
// Design:
//-----------------------------------------------------------------------------
bool MessageTestRemote::setupMailbox(const MailboxAddress& localAddress)
{
   // Create the Local Mailbox
   messageTestSMMailbox_ = LocalSMMailbox::createMailbox(localAddress);

   if (!messageTestSMMailbox_)
   {
      TRACELOG(ERRORLOG, MSGMGRLOG, "Unable to create local shared memory mailbox",0,0,0,0,0,0);
      return false;
   }//end if
...
void MessageTestRemote::processMailbox()
{
   ...
   MailboxProcessor mailboxProcessor(messageHandlerList_, *messageTestSMMailbox_);
  
   // Activate our Mailbox (here we start receiving messages into the Mailbox queue)
   if (messageTestSMMailbox_->activate() == ERROR)
   { 
      ...
      TRACELOG(ERRORLOG, MSGMGRLOG, "Failed to activate local shared memory mailbox",0,0,0,0,0,0);
   }//end if
...
void messageRemoteSendingTest()
{
   ...
   {
      TRACELOG(ERRORLOG, MSGMGRLOG, "Could not find Message Test LocalSM Local Mailbox",0,0,0,0,0,0);
      return;
   }//end if
   else
   {
      TRACELOG(DEBUGLOG, MSGMGRLOG, "Successfully found Message Test LocalSM Local Mailbox",0,0,0,0,0,0);
   }//end else
