...
static volatile char main_sccs_id[] __attribute__ ((unused)) = "@(#)MsgMgr Test 2"
    ...
    messageHandlerList_(NULL)
{
   ...
   distributedAddress.mailboxName = "MessageTestRemote";
   distributedAddress.inetAddress.set(7777, "127.0.0.1"); 
   distributedAddress.neid = "100000001";
...
void MessageTestRemote::processMailbox()
{
   ...
   // Start processing messages from the Mailbox queue. Here we start the Mailbox
   // with a thread pool of 5 threads to handle dequeuing.
   mailboxProcessor.processMailbox(5);
}//end processMailbox
...
int MessageTestRemote::processTestRemoteMessage(MessageBase* message)
{
   ...
   ostringstream ostr;
   ostr << "Thread (" << (unsigned long)ACE_Thread::self() << ") Received a Test Remote Message: " 
        << testRemoteMessage->toString() << ends;
...
int MessageTestRemote::processTest1Message(MessageBase* message)
{
   ...
   ostringstream ostr;
   ostr << "Thread (" << (unsigned long)ACE_Thread::self() << ") Received a Test1 Message: "
        << test1Message->toString() << ends;
   STRACELOG(DEBUGLOG, MSGMGRLOG, ostr.str().c_str());
   return OK;
...
int MessageTestRemote::processTimerMessage(MessageBase* message)
{
   ...
   ostringstream ostr;
   ostr << "Thread (" << (unsigned long)ACE_Thread::self() << ") Received a Timer Message: " << buf << ends;
   STRACELOG(DEBUGLOG, MSGMGRLOG, ostr.str().c_str());
...
void messageRemoteSendingTest()
{
   ...
   while (1)
   {
      ...
      // Create a Timer Message and schedule it. Here, we try using a non-recurring
      // Timer with no restartInterval specified.
      //
      // DO NOT BLINDLY COPY THIS INTO YOUR CODE WITHOUT UNDERSTANDING IT. Typically we would use the
      // OPM and have a pool of these timers.
      ACE_Time_Value timerDelay(5);  // 5 seconds to wait
      MessageTestTimerMessage* timerMessage = new MessageTestTimerMessage(sourceAddress, timerDelay);

      // Schedule the Timer Message with the MessageTestRemote Local mailbox
      ...
      //usleep (20);
      sleep (5);  // Here with 5 minutes sleep and 5 minutes scheduling we should be starting
                  // and consuming timers every 5 minutes.
   }//end while
...
int main(int argc, char* argv[])
{
   ...
   MessageTestRemote* messageTestRemote = new MessageTestRemote();
